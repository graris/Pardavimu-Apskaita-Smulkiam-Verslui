...
public class IrasoRedagavimoDialog extends JDialog{
	...
	public IrasoRedagavimoDialog(IrasuPerziurosPanele irasuPerziurosPanele, 
			int pasirinktaEilute, 
			String sqlTableName) {
	
		this.irasuPerziurosPanele = irasuPerziurosPanele;
		this.pasirinktaEilute = pasirinktaEilute;
	...
	private void init() {
		...
		mygtukas = new JButton("Iðsaugoti pakeitimus");
				...
				if(new ArTinkamasTipas(modelis.getValueAt(0, 0).toString())
					...
					if (sqlTableName.equals("Pirkejas")) {
						...
						new DBduomenys(irasuPerziurosPanele.getSt(), irasuPerziurosPanele.getDuomenuIrasai())
							.vykdykQuery(new SQLuzklausos().updatePirkejoDuomenys(
								irasuPerziurosPanele.getDuomenuIrasai().get(pasirinktaEilute).get(0),
					...
					if (sqlTableName.equals("Tiekejas")) {
						...
						new DBduomenys(irasuPerziurosPanele.getSt(), irasuPerziurosPanele.getDuomenuIrasai())
							.vykdykQuery(new SQLuzklausos().updateTiekejoDuomenys(
								irasuPerziurosPanele.getDuomenuIrasai().get(pasirinktaEilute).get(0),
								modelis.getValueAt(0, 0).toString(),
								modelis.getValueAt(0, 1).toString(),
								modelis.getValueAt(0, 2).toString(),
	...
	private void prepareTable() {
		...
		for (int i=0;i<irasuPerziurosPanele.getDuomenuIrasai().get(0).size(); i++) {
			...
			stulp[0] = irasuPerziurosPanele.getDuomenuIrasai().get(pasirinktaEilute).get(i);
